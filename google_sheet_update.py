...
def get_embedding(img_path):
    try:
        ...
        img = img.unsqueeze(0)  # Convert to batch format [1, 3, 160, 160]
    ...
    if boxes is not None and len(boxes) > 0:
        for box in boxes:
            ...
            if face_tensor is None:
                print("❌ No face extracted by MTCNN, skipping...")
                continue  # Skip if no face was extracted

            face_tensor = face_tensor.to(device)  # Move to GPU
            face_tensor = face_tensor.unsqueeze(0)  # Ensure batch format

            face_embedding = resnet(face_tensor).detach().cpu()  # Move back to CPU
